...
<span><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentcolor"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414.0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414.0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/></svg></span><label><select name=theme class="absolute appearance-none inset-0 w-full h-full opacity-0 dark:text-slate-500" @change.debounce=toggleTheme($event.target.value)><option value=none selected disabled hidden>default</option><option value=light>Light</option><option value=dark>Dark</option></select></label></div></div></div></div></div></div><main><div class="container mx-auto xl:max-w-screen-xl mt-8 px-2 flex justify-between space-x-5"><div class="md:w-3/4 overflow-hidden"><div class=space-y-5><h1 class="text-2xl font-bold dark:text-slate-300">删除文件夹中的node_modules子文件夹</h1><div class="flex text-sm space-x-5"><span class="flex items-center space-x-1"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentcolor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5A2 2 0 003 7v12a2 2 0 002 2z"/></svg><time>2021-09-18</time></span><ul class="flex items-center space-x-2"><li class="px-1 py-0.5 rounded text-white bg-emerald-500"><a href=https://zhujianghan.github.io/zjh-blog/tags/go>go</a></li></ul></div><div x-data="{
        theme: localStorage.theme === 'dark' ? 'dark' : 'light'
...
<span class="px-2 text-xs rounded-full text-slate-100 bg-emerald-500">16</span></a></li><li class="inline-block mt-3 mr-2"><a href=https://zhujianghan.github.io/zjh-blog/tags/jetbrains/ class="px-2 py-1 rounded bg-slate-200 dark:bg-slate-600"><span class=text-sm>jetbrains</span>
...
<span class="px-2 text-xs rounded-full text-slate-100 bg-emerald-500">3</span></a></li></ul></div></div></div></div></main><div class="container mx-auto xl:max-w-screen-xl px-2 mt-10 border-t">🍀</div><div x-data="{
    ...
    handleScroll(){
      ...
      if (this.show){
        let rate = document.body.scrollTopMax !== 0 ? document.body.scrollTop/document.body.scrollTopMax : document.documentElement.scrollTop/document.documentElement.scrollTopMax
